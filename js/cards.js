    ...
    },
{
  ...
  },
  {
    ...
    rarity: "稀有",
    category: "炎熱生物",
    image: "https://i.pinimg.com/736x/b1/96/0b/b1960bf87fad5d917deba7e4ced5991f.jpg",
    role: "毒爬行類",
...
(() => {
  ...
  const notify = (message) => {
    if (window.ShopSystem && typeof window.ShopSystem.showNotification === 'function') {
      window.ShopSystem.showNotification(message, 'info');
      return;
    }
    const el = document.createElement('div');
    el.style.cssText = 'position:fixed;top:20px;right:20px;z-index:99999;background:linear-gradient(135deg,#00ffff,#a259ff);color:#fff;padding:12px 16px;border-radius:12px;font-weight:900;box-shadow:0 0 18px rgba(0,0,0,0.35);';
    el.textContent = message;
    document.body.appendChild(el);
    setTimeout(() => el.remove(), 2500);
  };

  const getUsedCodeSet = () => {
    const used = safeJsonParse(localStorage.getItem('usedInviteCodes') || '[]', []);
    const set = new Set();
    (Array.isArray(used) ? used : []).forEach(x => {
  ...
  const getChestCodeSet = () => {
    const chests = safeJsonParse(localStorage.getItem('inviteCodeChests') || '[]', []);
    const set = new Set();
    (Array.isArray(chests) ? chests : []).forEach(x => {
      const c = String(x?.code || '').toUpperCase();
      if (c) set.add(c);
    });
    return set;
  ...
  const appendChests = (newChests) => {
    const current = safeJsonParse(localStorage.getItem('inviteCodeChests') || '[]', []);
    const arr = Array.isArray(current) ? current : [];
    const next = arr.concat(newChests);
    localStorage.setItem('inviteCodeChests', JSON.stringify(next));
  };
  ...
  const buildPanel = () => {
    ...
    overlay.id = 'sindyInviteAdminOverlay';
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.65);z-index:99998;display:flex;align-items:center;justify-content:center;padding:18px;';

    const panel = document.createElement('div');
    panel.style.cssText = 'width:min(560px,100%);background:rgba(10,20,40,0.92);border:2px solid rgba(0,255,255,0.65);border-radius:18px;box-shadow:0 0 28px rgba(0,255,255,0.18);backdrop-filter:blur(10px);padding:18px;color:#fff;font-family:system-ui, -apple-system, Segoe UI, Roboto, sans-serif;';

    panel.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
        <div style="font-size:18px;font-weight:900;color:#00ffff;text-shadow:0 0 10px rgba(0,255,255,0.35);">🛠️ 邀請碼後台發放</div>
        <button id="sindyInviteAdminClose" style="background:rgba(255,255,255,0.10);color:#fff;border:1px solid rgba(255,255,255,0.20);border-radius:10px;padding:6px 10px;cursor:pointer;font-weight:900;">關閉</button>
      </div>

      <div style="margin-top:12px;color:rgba(255,255,255,0.8);font-size:13px;line-height:1.6;">
        會從 <b>js/inviteCodes.json</b> 取出未使用且未領取的邀請碼，發放到 <b>邀請碼寶箱</b>（<code>inviteCodeChests</code>）。
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:14px;">
        <label style="display:flex;flex-direction:column;gap:6px;">
          <span style="font-weight:900;color:#00ffff;">面額（星星）</span>
          <select id="sindyInviteAdminStars" style="padding:10px 12px;border-radius:12px;border:1px solid rgba(0,255,255,0.25);background:rgba(0,0,0,0.25);color:#fff;outline:none;">
        ...
        <label style="display:flex;flex-direction:column;gap:6px;">
          <span style="font-weight:900;color:#00ffff;">數量</span>
          <input id="sindyInviteAdminCount" type="number" min="1" max="500" step="1" value="1" style="padding:10px 12px;border-radius:12px;border:1px solid rgba(0,255,255,0.25);background:rgba(0,0,0,0.25);color:#fff;outline:none;" />
        </label>
      </div>

      <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end;margin-top:14px;">
        <button id="sindyInviteAdminGrant" style="background:linear-gradient(135deg,#00ffff,#a259ff);color:#000;border:none;border-radius:12px;padding:10px 14px;font-weight:900;cursor:pointer;min-width:140px;">發放到寶箱</button>
      </div>

      <div id="sindyInviteAdminResult" style="margin-top:12px;white-space:pre-wrap;font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace;font-size:12px;color:rgba(255,255,255,0.85);background:rgba(0,0,0,0.25);border:1px solid rgba(0,255,255,0.15);border-radius:12px;padding:10px;display:none;"></div>
    `;
    ...
    panel.querySelector('#sindyInviteAdminClose').addEventListener('click', close);

    const btn = panel.querySelector('#sindyInviteAdminGrant');
    const starsEl = panel.querySelector('#sindyInviteAdminStars');
    const countEl = panel.querySelector('#sindyInviteAdminCount');
    const resultEl = panel.querySelector('#sindyInviteAdminResult');

    btn.addEventListener('click', async () => {
      const stars = parseInt(starsEl.value, 10) || 0;
      const count = Math.max(1, Math.min(500, parseInt(countEl.value, 10) || 1));
      if (!stars) return;
      ...
      try {
        const usedSet = getUsedCodeSet();
        const chestSet = getChestCodeSet();
        const all = await fetchInviteCodes();
        const pool = all
          .filter(x => parseInt(x?.stars, 10) === stars)
          .map(x => ({
            code: String(x?.code || '').toUpperCase(),
            stars: parseInt(x?.stars, 10) || 0,
            used: Boolean(x?.used)
          }))
          .filter(x => x.code && x.stars && !x.used && !usedSet.has(x.code) && !chestSet.has(x.code));

        if (pool.length === 0) {
          resultEl.textContent = '找不到可用的邀請碼（可能已用完 / 已領取 / JSON 未載入成功）。';
          notify('沒有可用邀請碼');
          return;
        }
        ...
        const newChests = take.map((x, i) => ({
          setId: `admin_grant_${stars}_${Date.now()}_${i}`,
          categoryName: '後台發放',
          stars: x.stars,
          code: x.code,
          earnedAt: now,
        ...
        appendChests(newChests);
        resultEl.textContent = `已發放：${newChests.length} 個\n面額：${stars}\n\n${newChests.map(c => c.code).join('\n')}`;
        notify(`已發放 ${newChests.length} 個邀請碼寶箱`);
      } catch (e) {
        resultEl.textContent = `失敗：${e && e.message ? e.message : String(e)}`;
        notify('發放失敗');
      } finally {
        btn.disabled = false;
        btn.style.opacity = '1';
